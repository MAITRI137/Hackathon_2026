...
import { createFuelLog } from "@/app/(protected)/actions";
...
export function ReceiptOcr({
  ...
  const [progress, setProgress] = useState(0),
    [raw, setRaw] = useState(""),
    [parsed, setParsed] = useState<ReturnType<typeof parseReceiptText> | null>(
      null
    ),
  ...
  return (
    <form action={createFuelLog} className="grid gap-3">
      ...
      {raw && (
        <details open className="rounded-2xl bg-muted p-3">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="grid gap-1 text-xs font-bold">
          Litres
          <input
            key={`litres-${parsed?.litres}`}
            name="litres"
            type="number"
            step=".01"
            defaultValue={parsed?.litres}
            required
            className={fieldClass}
          />
        </label>
        ...
        <label className="grid gap-1 text-xs font-bold">
          ...
          <input
            name="odometer"
            type="number"
            defaultValue={matched?.odometer || vehicles[0]?.odometer}
            required
            className={fieldClass}
          />
        </label>
      ...
      <p className="text-xs text-muted-foreground">
        OCR never saves automatically. Review and correct every field before
        accepting.
      </p>
