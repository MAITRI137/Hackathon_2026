...
> = {
  vehicles: async () => ({
    ...
    rows: (await db.vehicle.findMany({ orderBy: { name: "asc" } })).map((v) => [
  ...
  drivers: async () => ({
    ...
    rows: (await db.driver.findMany({ orderBy: { name: "asc" } })).map((d) => [
      d.name,
      d.employeeId,
      d.licenceNumber,
      d.licenceCategory,
  ...
  maintenance: async () => ({
    ...
    rows: (
      await db.maintenanceLog.findMany({
  ...
  compliance: async () => ({
    ...
    rows: (
      await db.complianceAlert.findMany({ orderBy: { createdAt: "desc" } })
    ).map((a) => [
      a.severity,
      a.entityType,
      a.message,
...
export async function GET(
  ...
  await db.auditLog.create({
    data: {
      ...
      action: "CSV_EXPORT",
      entityType: "Report",
      details: `${entity} (${rows.length} rows)`,
    },
  });
