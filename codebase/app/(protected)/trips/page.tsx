...
import { cancelTrip, completeTrip, createTrip, dispatchTrip } from "../actions";
...
export default async function TripsPage({
  ...
  const user = await requirePermission("read:trips");
  const canManage = hasPermission(user, "manage:trips");
  ...
  const recommendation = recommendDispatch(
    ...
    vehicles.map((v) => ({
      ...
      fuelEfficiency: v.type.includes("Truck") ? 6 : 12,
      documentsValid: [
        v.insuranceExpiry,
        ...
        v.fitnessExpiry,
        v.permitExpiry,
      ].every((d) => d >= completion),
      maintenanceHealthy: v.status !== "IN_SHOP",
    })),
    drivers.map((d) => ({
      id: d.id,
      name: d.name,
  ...
  const tomorrow = new Date(Date.now() + 86_400_000).toISOString().slice(0, 16),
    dayAfter = new Date(Date.now() + 2 * 86_400_000).toISOString().slice(0, 16);
  return (
    <div>
      <PageHeader
        eyebrow="Live operations"
        title="Trip Dispatcher"
        description="Validate capacity, compliance, schedules and profitability before resources move."
      />
      ...
      <section className="mb-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          value={`₹${trips.reduce((s, t) => s + t.expectedRevenue, 0).toLocaleString("en-IN")}`}
        />
      </section>
          ...
          <Form action={createTrip} className={cardClass}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <label className="grid gap-1 text-xs font-bold">
                ...
                <select
                  name="vehicleId"
                  defaultValue={recommendation.best?.vehicle.id}
                  className={fieldClass}
                >
                  {vehicles
              ...
              <label className="grid gap-1 text-xs font-bold">
                ...
                <select
                  name="driverId"
                  defaultValue={recommendation.best?.driver.id}
                  className={fieldClass}
                >
                  {drivers.map((d) => (
        ...
        <section className={cardClass}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="mt-1 text-xl font-semibold">
                {recommendation.best
                  ? `${recommendation.best.vehicle.name} + ${recommendation.best.driver.name}`
                  : "No eligible pair"}
              </h2>
            </div>
            {recommendation.best && (
            ...
            <>
              ...
              <div className="grid grid-cols-3 gap-2 rounded-2xl bg-muted p-3 text-center text-xs">
                ...
                <div>
                  ...
                  <strong className="block text-base">
                    ₹
                    {recommendation.best.breakEvenRevenue.toLocaleString(
                      "en-IN"
                    )}
                  </strong>
                </div>
                <div>
                  ...
                  <strong className="block text-base text-primary">
                    ₹
                    {recommendation.best.estimatedMargin.toLocaleString(
                      "en-IN"
                    )}
                  </strong>
                </div>
          ...
          <details className="mt-4">
            <summary className="cursor-pointer text-sm font-bold text-primary">
              Rejected candidates ({recommendation.rejected.length})
            </summary>
            ...
            <div>
              ...
              <dl className="mt-5 grid gap-3 text-sm">
                <div>
                  ...
                  <dd className="font-bold">{route.distanceKm} km</dd>
                </div>
                <div>
            ...
            {trips.map((t) => (
              <tr key={t.id} className="border-b align-top last:border-0">
                ...
                <td className="px-5 py-4 text-xs">
                  {t.plannedStart.toLocaleString("en-IN")}
                </td>
                <td className="px-5 py-4">
                  {t.vehicle?.name || "Unassigned"}
                  <span className="block text-xs text-muted-foreground">
                    {t.driver?.name || "Unassigned"}
                  </span>
                </td>
                <td className="px-5 py-4">{t.cargoWeight} kg</td>
                <td className="px-5 py-4">
                  ₹{t.expectedRevenue.toLocaleString("en-IN")}
                  <span className="block text-xs text-primary">
                    ₹{t.estimatedMargin.toLocaleString("en-IN")} margin
                  </span>
                </td>
                <td className="px-5 py-4">
